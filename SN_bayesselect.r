...
#(addition)deltab:name of the asymetry parameter vector for random effects
#(addition)deltae:name of the asymetry parameter vector for random error 


mc_sam=results_s
resp=simdata$y
clust=simdata$id
X=cbind(1,simdata$x,simdata$t)
Z=c(rep(1,length(simdata$x)))
betas=c("beta[1]","beta[2]","beta[3]")
var_y="var_e"
varZ="sigma_b"
deltae="deltae"
deltab="deltab"

SN_bayesselect<- function(mc_sam, resp, clust, X, Z, betas, var_y, varZ,deltae,deltab){
    ...
    sam_deltae<-as.matrix(mc_sam[,deltae])
    ...
    if(ncol(Z)>1)
    {
      ...
      sam_deltab <- as.matrix(mc_sam[,grep(paste0(deltab,"\\["),names_mcmc)]) #(addition)
    
      #sam_varE <- as.matrix(mc_sam[,grep(paste0(varE,"\\["),names_mcmc)]) #(addition)
    }else{
      sam_varz <- as.matrix(mc_sam[,varZ])
      sam_deltab <- as.matrix(mc_sam[,deltab])                 #(addition)
      #sam_varE <- as.matrix(mc_sam[,varE])                 #(addition)
    }
    ...
    sam_deltae_p<- mean(sam_deltae)#(#addition)
    sam_deltab_p <- colMeans(sam_deltab) #(#addition)
    logp_yi<- rep(0,nK)
    logp_yib<- rep(0,nK)
    
    loopi<-function(i,sam_mean,sam_meanb,nK,clustl,Z,resp,sam_varz_p,sam_vary_p,sam_varz,sam_vary,sam_deltab_p,sam_deltae_p) { #Loop for observations
     ...
     #chech Zi
      ...
      if (as.numeric(is.null(nrow(Z)))==1){
      Z_i=Z[unlist(clustl[i])]
      }else
      ...
      if(ncol(Z)==1){
        Lam_i<-1/(1/sam_varz_p+t(Z_i)%*%diag(sam_vary_p,nrow(Z_i))%*%Z_i)
        } else{Lam_i<-inv(inv(as.matrix(sam_varz_p, ncol=ncol(Z)))+t(Z_i)%*%as.matrix(diag(sam_vary_p,nrow(Z_i)))%*%Z_i)}
      
       mu_1i<-Lam_i%*%t(Z_i)%*%inv(diag(sam_vary_p,nrow(Z_i)))
       A<-t(as.matrix(rep(sam_deltae_p, nrow(Z_i)),nrow=1)) %*%inv(sqrtm(as.matrix(diag(sam_vary_p,nrow(Z_i)))))
       B<- rep(0,nrow(Z_i))              
       mu_2i<- rbind(A,B)
       
      C<- t(as.matrix(rep(sam_deltae_p, nrow(Z_i)),nrow=1))%*%as.matrix(diag(sam_vary_p, nrow(Z_i)))%*%Z_i
      
      if(ncol(Z)==1){
       D<-t(matrix(rep(sam_deltab_p, ncol=ncol(Z)),nrow=1))/sam_varz_p^0.5
      } else{ D<-t(matrix(rep(sam_deltab_p, ncol=ncol(Z)),nrow=1))%*%inv(sqrtm(matrix(sam_varz_p, ncol=ncol(Z))))}
      
      LAM_i<-rbind(C,D)
      upper <- as.numeric((mu_2i-LAM_i%*% mu_1i)%*%(resp_i[indav]-mu_yi_p[indav]))
      sigma<-diag(2)%*%+LAM_i%*% Lam_i%*%t(LAM_i)
      
        cum<-pmvnorm(lower=rep(-Inf,length(upper)), upper=upper,mean=rep(0, length(upper)),sigma=sigma)
      logL_pm <- -0.5%*%(dmvnorm(resp_i[indav], mu_yi_p[indav], sigma=as.matrix(marvar_p[indav,indav]),log=TRUE))+ sum(log(cum))
      

      ########logL_pm <- -0.5%*%log(sum((diag(sam_vary_p, nrow(Z_i)))))-(0.5%*%sum(dmvnorm(resp_i[indav], mu_yi_p[indav],sigma=as.matrix(marvar_p[indav,indav]),log=FALSE)+sum(log(cum))
                   
  
      #rep(-Inf, m)
        like<-function(k,Z_i,sam_varz,Z,sam_vary,resp_i,mu_yi,mub_yi,indav){
        
          #Loop for MCMC iterations
          marvar <- Z_i %*% matrix(sam_varz[k,], ncol=ncol(Z_i)) %*% t(Z_i) + diag(sam_vary[k], nrow(Z_i)) ####check with Adrian
 ...
 resloopi=sapply(c(seq(1,nclus,1)),loopi,sam_mean=sam_mean,sam_meanb=sam_meanb,nK=nK,clustl=clustl,Z=Z,resp=resp,sam_varz_p=sam_varz_p,sam_vary_p=sam_vary_p,sam_varz=sam_varz,sam_vary=sam_vary,sam_deltae_p=sam_deltae_p,sam_deltab_p=sam_deltab_p)
 ...
 pvb<-var(sumres[(11+mid):(10+mid*2)])/2
    ...
    Dev_pm <- -2*sumres[5] #sum(logL_pm) Check
